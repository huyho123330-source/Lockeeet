...
def handle_all_text_messages(message):
    ...
    # 1. Xử lý trạng thái nhập mã quà tặng
    if state == "WAITING_FOR_PROMO_CODE":
        update_user_state(user_id, None)
        code_input = text.upper()
        conn = sqlite3.connect("bot_vip_database.db", check_same_thread=False)
        cursor = conn.cursor()
        cursor.execute("SELECT money, uses_left FROM promo_codes WHERE code = ?", (code_input,))
        row = cursor.fetchone()
        
        if row:
            money_reward, uses_left = row[0], row[1]
            if uses_left > 0:
                new_uses = uses_left - 1
                if new_uses <= 0:
                    cursor.execute("DELETE FROM promo_codes WHERE code = ?", (code_input,))
                else:
                    cursor.execute("UPDATE promo_codes SET uses_left = ? WHERE code = ?", (new_uses, code_input))
                conn.commit()
                conn.close()
                
                new_balance = user_data[0] + money_reward
                update_user_balance(user_id, new_balance)
                bot.reply_to(message, f"🎉 <b>NHẬP MÃ THÀNH CÔNG!</b>\n💰 Bạn nhận được: <code>+{money_reward:,.0f} VNĐ</code>\n📦 Lượt dùng còn lại của mã: <code>{new_uses}</code>\n💳 Số dư mới: <code>{new_balance:,.0f} VNĐ</code>", parse_mode="HTML", reply_markup=get_main_keyboard(user_id))
            else:
                conn.close()
                bot.reply_to(message, "❌ Mã quà tặng đã hết lượt sử dụng!", reply_markup=get_main_keyboard(user_id))
        else:
            conn.close()
            bot.reply_to(message, "❌ Mã quà tặng không tồn tại!", reply_markup=get_main_keyboard(user_id))
        return
